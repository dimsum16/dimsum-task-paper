\resizebox{\textwidth}{!}{
\begin{tabular}{@{}r@{~~}l@{~~}l@{~~}>{\hspace{15pt}}l@{~~}r@{~~}r@{~~}r@{~}>{\smaller}r@{~~}r@{~~}r@{~}r@{~~}r@{~}r@{}}
...
\caption[Annotated datasets]{Annotated datasets: status of lexical semantic annotations 
(retained, revised, or newly annotated for this task) per subcorpus; word token and MWE instance counts; 
...
number of units labeled with a noun supersense, and proportion that are MWEs; 
 likewise for verb supersenses.\endgraf\setlength{\parindent}{1em}
Additional statistics relatively consistent across domains: 
MWEs per word: mean/median .055 (lowest: TED, .044; highest: STREUSLE, .090).
Supersenses per word: mean/median 0.3.
Just 8 MWEs contain more than one gap (all in STREUSLE or Ritter).}
