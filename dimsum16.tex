...
\item[(2)] \raggedright I googled restaurants in the area and {\color{red}Fuji Sushi} {\color{blue}came up} and reviews were great so I {\color{green}made a} {\color{purple}carry out} {\color{green}order} of : L 17 .
\end{itemize}
...
\paragraph{Training}
The review part of the training data  consists of a 55,000-token portion of the English Web Treebank \citep[EWTB;][]{ewtb} 
made up of 723~online user reviews for services (such as restaurants or beauticians).\longversion{\footnote{Each review is a separate document; 
...
\citet{schneider-14-corpus} recently manually annotated the entire data  for heterogeneous MWEs 
and noun supersenses (following the conventions of \citealp{schneider-12}), then for verb supersenses, and released it as the CMWE Corpus.
These annotations will be publicly released at \url{http://www.ark.cs.cmu.edu/LexSem/} 
(Google and LDC have granted us permission to redistribute the source text and gold POS tags from the EWTB reviews). 

\paragraph{Test}
The test portion comprises  340 sentences (6,357 tokens) from the online review site Trustpilot, a subset of the data used in \citet{hovy-2015age} (the website as a general resource was described in \citet{hovy-2015trustpilot}). The reviews were chosen to control for age, gender, and location, and contained gold POS tags. The data was further annotated for supersenses and MWE by 
